...
// node_modules/.pnpm/slug@11.0.0/node_modules/slug/slug.js
var base64;
...
// src/cli/utils.ts
function getOutputFileName(file, httpType) {
  if (file === "http.ts.hbs" && httpType === "convex")
    return "http.ts";
  if (file === "http.hono.ts.hbs" && httpType === "hono")
    return "http.ts";
  return file.replace(/\.hono\.ts\.hbs$/, ".ts").replace(/\.ts\.hbs$/, ".ts").replace(/\.hbs$/, "");
}
async function renderAndCopyHbs(src, dest, context = {}) {
  const template = await import_fs_extra.default.readFile(src, "utf8");
  const compiled = import_handlebars.default.compile(template);
  await import_fs_extra.default.outputFile(dest, compiled(context));
}
async function copyAndRenderTemplates(srcDir, destDir, context, httpType, options = {}) {
  ...
  for (const entry of entries) {
    ...
    const destPath = import_path.default.join(destDir, outName);
    if (options.isConvexRoot && entry.name === "workos" && entry.isDirectory()) {
      await import_fs_extra.default.ensureDir(destPath);
      const workosEntries = await import_fs_extra.default.readdir(srcPath, { withFileTypes: true });
      for (const workosEntry of workosEntries) {
        const workosEntrySrc = import_path.default.join(srcPath, workosEntry.name);
        if (httpType === "hono" && workosEntry.name === "webhooks-hono" || httpType === "convex" && workosEntry.name === "webhooks") {
          const webhooksDest = import_path.default.join(destPath, "webhooks");
          ...
          await copyAndRenderTemplates(
            workosEntrySrc,
            import_path.default.join(destPath, workosEntry.name),
            context,
            httpType
          );
        } else if (workosEntry.isFile() && workosEntry.name.endsWith(".hbs")) {
          const outName2 = getOutputFileName(workosEntry.name, httpType);
          const outPath = import_path.default.join(destPath, outName2);
          await renderAndCopyHbs(workosEntrySrc, outPath, context);
        } else if (workosEntry.isFile()) {
          await import_fs_extra.default.copy(workosEntrySrc, import_path.default.join(destPath, workosEntry.name), {
            overwrite: false,
            errorOnExist: false
          });
        }
    ...
    if (options.onlyHttp && (entry.name === "http.ts.hbs" && httpType !== "convex" || entry.name === "http.hono.ts.hbs" && httpType !== "hono")) {
      continue;
    }
    if (entry.isDirectory()) {
      ...
      await import_fs_extra.default.copy(srcPath, destPath, { overwrite: false, errorOnExist: false });
    }
  }
}
function detectPackageManager(cwd) {
  if (import_fs_extra.default.existsSync(import_path.default.join(cwd, "pnpm-lock.yaml")))
    return "pnpm";
  if (import_fs_extra.default.existsSync(import_path.default.join(cwd, "yarn.lock")))
    return "yarn";
  if (import_fs_extra.default.existsSync(import_path.default.join(cwd, "bun.lockb")))
    return "bun";
  if (import_fs_extra.default.existsSync(import_path.default.join(cwd, "package-lock.json")))
    return "npm";
  return "npm";
}
function installDeps(pkgManager, deps, dev = false) {
  if (!deps.length)
    return;
  let cmd = "";
  switch (pkgManager) {
...
async function getUserPrompts() {
  ...
  let projectName = "";
  if (!import_fs_extra.default.existsSync("next.config.ts")) {
    const { promptPackageManager } = await (0, import_prompts.default)({
      type: "select",
    ...
    const { promptProjectName } = await (0, import_prompts.default)({
      type: "text",
  ...
  const projectNameParsed = slug_default(projectName, {
    lower: true,
    trim: true,
    replacement: "-"
  });
  if (import_fs_extra.default.existsSync(projectNameParsed)) {
    console.error(
      `A project with the name ${projectNameParsed} already exists in the current directory. Please choose a different name.`
    );
    process.exit(1);
  }
  const { workosApiKey } = await (0, import_prompts.default)({
    type: "text",
    name: "workosApiKey",
    message: "Enter your WorkOS API key:"
  });
  const { workosClientId } = await (0, import_prompts.default)({
    type: "text",
    name: "workosClientId",
    message: "Enter your WorkOS Client ID:"
  });
  const { httpType } = await (0, import_prompts.default)({
    type: "select",
    name: "httpType",
  ...
  const { email } = await (0, import_prompts.default)({
    type: "select",
    name: "email",
  ...
  let emailFrom = null;
  if (email === "yes") {
    const { promptResendApiKey } = await (0, import_prompts.default)({
      type: "text",
      name: "promptResendApiKey",
      message: "Enter your Resend API key:"
    });
    const { promptEmailFrom } = await (0, import_prompts.default)({
      type: "text",
...
function installDependencies({
  ...
  if (!import_fs_extra.default.existsSync("components.json")) {
    switch (packageManager) {
      case "pnpm":
        (0, import_child_process.execSync)(`pnpm dlx shadcn@latest init`, { stdio: "inherit" });
        break;
      case "npm":
        (0, import_child_process.execSync)(`npx shadcn@latest init`, { stdio: "inherit" });
        break;
      case "bun":
        (0, import_child_process.execSync)(`bunx --bun shadcn@latest init`, { stdio: "inherit" });
        break;
      case "yarn":
        (0, import_child_process.execSync)(`yarn shadcn@latest init`, { stdio: "inherit" });
        break;
      default:
        throw new Error(`Unsupported package manager: ${packageManager}`);
    }
...
async function copyAndRenderFolders(srcDir, destDir, context, httpType, options = {}) {
  const stat = await import_fs_extra.default.stat(srcDir);
  if (!stat.isDirectory()) {
...
async function copyAndRenderFiles(tasks) {
  for (const { src, dest, context = {} } of tasks) {
    if (src.endsWith(".hbs")) {
      await renderAndCopyHbs(src, dest, context);
    } else {
      await import_fs_extra.default.copy(src, dest, { overwrite: false, errorOnExist: false });
    }
  }
}
async function patchRootLayoutProviders(layoutPath) {
  let content = await import_fs_extra.default.readFile(layoutPath, "utf8");
  ...
  await import_fs_extra.default.writeFile(layoutPath, content, "utf8");
}
async function patchTsconfigStrictOptions(tsconfigPath) {
  const content = await import_fs_extra.default.readFile(tsconfigPath, "utf8");
  let json;
  try {
  ...
  await import_fs_extra.default.writeFile(tsconfigPath, JSON.stringify(json, null, 2) + "\n", "utf8");
}
async function patchRootLayoutAttributes(layoutPath) {
  let content = await import_fs_extra.default.readFile(layoutPath, "utf8");
  content = content.replace(/<html([^>]*)>/, (_match, attrs) => {
    ...
    if (/className=/.test(newAttrs)) {
      newAttrs = newAttrs.replace(/className="([^"]*)"/, (_m, classes) => {
  ...
  content = content.replace(/<body([^>]*)>/, (_match, attrs) => {
    ...
    if (/className=/.test(newAttrs)) {
      newAttrs = newAttrs.replace(/className=({`[^`]*`}|"[^"]*")/, (_m, value) => {
        let newValue = value;
        if (value.startsWith("{`") && value.endsWith("`}")) {
          let inner = value.slice(2, -2).trim();
          if (!inner.includes("h-full"))
            inner += " h-full";
          if (!inner.includes("w-full"))
            inner += " w-full";
          if (!inner.includes("bg-background"))
            inner += " bg-background";
          newValue = `{\`${inner.trim()}\`}`;
...
async function main() {
  ...
  const destConvexJson = import_path2.default.join(process.cwd(), "convex.json");
  if (!import_fs_extra2.default.existsSync(destConvexJson)) {
    const templateConvexJson = import_path2.default.join(__dirname, "../src/convex.json");
    await import_fs_extra2.default.copy(templateConvexJson, destConvexJson, { overwrite: false });
  }
  const schemasSrc = import_path2.default.join(__dirname, "../src/schemas");
  const schemasDest = import_path2.default.join(srcDir, "schemas");
  await copyAndRenderFolders(schemasSrc, schemasDest, {}, httpType);
  const convexSrc = import_path2.default.join(__dirname, "../src/convex");
  const convexDest = import_path2.default.join(srcDir, "convex");
  await copyAndRenderFolders(convexSrc, convexDest, {}, httpType, {
    isConvexRoot: true,
    onlyHttp: true
  });
  const nextjsSrc = import_path2.default.join(__dirname, "../src/nextjs");
  const oauthRouteSrc = import_path2.default.join(nextjsSrc, "app/api/auth/oauth");
  const oauthRouteDest = import_path2.default.join(srcDir, "app/api/auth/oauth");
  await copyAndRenderFolders(oauthRouteSrc, oauthRouteDest, {}, httpType);
  const signInRouteSrc = import_path2.default.join(nextjsSrc, "app/api/auth/otp/sign-in");
  const signInRouteDest = import_path2.default.join(srcDir, "app/api/auth/otp/sign-in");
  await copyAndRenderFolders(signInRouteSrc, signInRouteDest, {}, httpType);
  const requestSignInRouteSrc = import_path2.default.join(
    nextjsSrc,
    manageEmailsWithResend ? "app/api/auth/otp/request-sign-in-with-resend" : "app/api/auth/otp/request-sign-in"
  );
  const requestSignInRouteDest = import_path2.default.join(srcDir, "app/api/auth/otp/request-sign-in");
  ...
  const nextjsAppSrc = import_path2.default.join(nextjsSrc, "app/(app)");
  const nextjsAppDest = import_path2.default.join(srcDir, "app/(app)");
  await copyAndRenderFolders(nextjsAppSrc, nextjsAppDest, {}, httpType);
  const nextjsAuthSrc = import_path2.default.join(nextjsSrc, "app/auth");
  const nextjsAuthDest = import_path2.default.join(srcDir, "app/auth");
  await copyAndRenderFolders(nextjsAuthSrc, nextjsAuthDest, {}, httpType);
  const publicSrc = import_path2.default.join(nextjsSrc, "public");
  const publicDest = import_path2.default.join(process.cwd(), "public");
  await copyAndRenderFolders(publicSrc, publicDest, {}, httpType);
  const libSrc = import_path2.default.join(nextjsSrc, "lib");
  const libDest = import_path2.default.join(srcDir, "lib");
  await copyAndRenderFolders(libSrc, libDest, {}, httpType);
  const componentsSrc = import_path2.default.join(nextjsSrc, "components");
  const componentsDest = import_path2.default.join(srcDir, "components");
  await copyAndRenderFolders(componentsSrc, componentsDest, {}, httpType);
  const hooksSrc = import_path2.default.join(nextjsSrc, "hooks");
  const hooksDest = import_path2.default.join(srcDir, "hooks");
  await copyAndRenderFolders(hooksSrc, hooksDest, {}, httpType);
  const queriesSrc = import_path2.default.join(nextjsSrc, "queries");
  const queriesDest = import_path2.default.join(srcDir, "queries");
  await copyAndRenderFolders(queriesSrc, queriesDest, {}, httpType);
  const middlewareSrc = import_path2.default.join(nextjsSrc, "middleware.ts.hbs");
  const middlewareDest = import_path2.default.join(srcDir, "middleware.ts");
  await copyAndRenderFiles([{ src: middlewareSrc, dest: middlewareDest, context: {} }]);
  const faviconSrc = import_path2.default.join(nextjsSrc, "app/favicon.ico");
  const faviconDest = import_path2.default.join(srcDir, "app/favicon.ico");
  await copyAndRenderFiles([{ src: faviconSrc, dest: faviconDest, context: {} }]);
  const homePageSrc = import_path2.default.join(nextjsSrc, "app/page.tsx.hbs");
  const homePageDest = import_path2.default.join(srcDir, "app/page.tsx");
  await copyAndRenderFiles([{ src: homePageSrc, dest: homePageDest, context: {} }]);
  const layoutPath = import_path2.default.join(srcDir, "app/layout.tsx");
  await patchRootLayoutProviders(layoutPath);
  await patchRootLayoutAttributes(layoutPath);
  const tsconfigPath = import_path2.default.join(process.cwd(), "tsconfig.json");
  await patchTsconfigStrictOptions(tsconfigPath);
  const randomSecret = (0, import_child_process2.execSync)("openssl rand -base64 32").toString().trim();
...
`;
  const envPath = import_path2.default.join(process.cwd(), ".env.local");
  await import_fs_extra2.default.appendFile(envPath, envLocalContent, { encoding: "utf8" });
